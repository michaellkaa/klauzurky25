  ...
  <main class="max-w-3xl mx-auto p-8 font-sans text-gray-900">
    ...
    <article v-else-if="faculty" class="space-y-8">
      ...
      <section class="relative rounded-md overflow-hidden border border-gray-200">
        <img v-if="faculty.banner_url" :src="faculty.banner_url" alt="Banner fakulty" class="w-full h-48 object-cover"
          loading="lazy" />
        <div class="absolute top-4 right-4">
          <HeartButton :type="'faculty'" :id="faculty.id" />
        </div>
        <nav class="absolute top-4 left-4 flex gap-4 text-gray-600 text-xl">
          <a v-if="faculty.facebook_url" :href="faculty.facebook_url" target="_blank" rel="noopener"
            aria-label="Facebook" class="hover:text-blue-600 transition">
            <svg class="w-5 h-5 fill-current" viewBox="0 0 24 24">
              <path
                d="M22 12a10 10 0 1 0-11.5 9.87v-6.99H8.09v-2.88h2.41v-2.2c0-2.39 1.43-3.7 3.63-3.7 1.05 0 2.15.18 2.15.18v2.37h-1.21c-1.2 0-1.57.74-1.57 1.5v1.85h2.67l-.43 2.88h-2.24v6.99A10 10 0 0 0 22 12z" />
            </svg>
          </a>
          <a v-if="faculty.instagram_url" :href="faculty.instagram_url" target="_blank" rel="noopener"
          ...
          <a v-if="faculty.twitter_url" :href="faculty.twitter_url" target="_blank" rel="noopener" aria-label="Twitter"
            class="hover:text-sky-500 transition">
            <svg class="w-5 h-5 fill-current" viewBox="0 0 24 24">
              <path
                d="M8 19c7.5 0 11.6-6.3 11.6-11.7 0-.18 0-.35-.01-.52A8.36 8.36 0 0 0 22 5.6a8.22 8.22 0 0 1-2.36.64 4.13 4.13 0 0 0 1.8-2.27 8.26 8.26 0 0 1-2.6.99 4.11 4.11 0 0 0-7 3.75A11.67 11.67 0 0 1 3 4.9a4.1 4.1 0 0 0 1.27 5.48 4.08 4.08 0 0 1-1.86-.5v.05a4.11 4.11 0 0 0 3.3 4 4.07 4.07 0 0 1-1.85.07 4.11 4.11 0 0 0 3.83 2.84A8.24 8.24 0 0 1 4 17.8a11.6 11.6 0 0 0 7.9 2.3" />
            </svg>
          </a>
        </nav>
      </section>
      ...
      <section class="grid grid-cols-1 sm:grid-cols-2 gap-y-2 text-sm text-gray-600 border-t border-gray-200 pt-6">
        ...
        <p v-if="faculty.website"><strong>Web:</strong> <a :href="faculty.website" target="_blank" rel="noopener"
            class="text-blue-600 hover:underline break-words">{{ faculty.website }}</a></p>
        <p v-if="faculty.email"><strong>Email:</strong> <a :href="`mailto:${faculty.email}`"
            class="text-blue-600 hover:underline">{{ faculty.email }}</a></p>
      ...
      <section class="mt-6 space-y-2 text-sm text-gray-600 border-t border-gray-200 pt-6">
        <p v-if="faculty.application_link"><strong>Přihláška:</strong> <a :href="faculty.application_link"
            target="_blank" rel="noopener" class="text-blue-600 hover:underline">Podat přihlášku</a></p>
        <p v-if="faculty.admission_notes"><strong>Poznámky k přijímacímu řízení:</strong> {{ faculty.admission_notes }}
        </p>
        <p v-if="faculty.open_day_dates"><strong>Dny otevřených dveří:</strong> {{ faculty.open_day_dates }}</p>
        <p v-if="faculty.open_day_url"><a :href="faculty.open_day_url" target="_blank" rel="noopener"
            class="text-blue-600 hover:underline">Více o dnech otevřených dveří</a></p>
        ...
        <p v-if="faculty.application_deadlines"><strong>Termíny podání přihlášek:</strong> {{
          faculty.application_deadlines }}</p>
      </section>
