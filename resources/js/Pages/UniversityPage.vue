...
<template>
  ...
  <main class="max-w-3xl mx-auto p-8 font-sans text-black">
    ...
    <div v-else-if="university" class="space-y-10">
      <!-- banner cast -->
      <section v-if="university.banner_url"
        class="relative rounded-xl overflow-hidden border border-gray-300 shadow-sm mb-6">
        ...
        <nav class="absolute top-4 left-4 flex gap-4 text-gray-700 text-lg font-bold">
          ...
          <a v-if="university.instagram" :href="university.instagram" target="_blank" rel="noopener"
            aria-label="Instagram" class="hover:text-black transition">
            IG
          </a>
          <a v-if="university.twitter" :href="university.twitter" target="_blank" rel="noopener" aria-label="Twitter"
            class="hover:text-black transition">
            TW
          </a>
        </nav>
      ...
      <section
        class="grid grid-cols-1 sm:grid-cols-2 gap-y-2 text-sm text-gray-800 border-t border-gray-300 pt-6 font-medium">
        <p v-if="university.website"><span class="font-bold uppercase">Web: </span>
          <a :href="university.website" target="_blank" rel="noopener"
            class="text-black font-semibold hover:underline break-words">
            {{ university.website }}
          </a>
        </p>
